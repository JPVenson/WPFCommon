...
namespace JPB.Communication.Shared
{
    public class NetworkBag<T> : NetworkValueBag<T>
        where T : class,
            ...
            new()
    {
        public NetworkBag(ushort port, string guid)
            : base(port, guid)
        {
            base.TCPNetworkReceiver.RegisterMessageBaseInbound(pPullPropertyChanged, NetworkCollectionProtocol.CollectionUpdateItem);
        }
